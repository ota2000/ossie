...
> **NULL-safe merge (normative addition).** The merge step below MUST use **NULL-safe equality** on the shared key(s) — e.g. `a.customer_id IS NOT DISTINCT FROM b.customer_id` rather than plain `=` (see the Null-Safe Comparison form in the [Ossie Expression Language](https://github.com/apache/ossie/blob/main/core-spec/expression_language.md) proposal). A plain-equality `FULL OUTER JOIN` never matches two rows that both have NULL in the join key (ANSI `NULL = NULL` is unknown, not true), so if the shared dimension is nullable and both facts have a NULL-keyed group, a plain-equality join emits **two** output rows for that group instead of one merged row — one fact's measure would sit next to a NULL for the other fact on *both* rows, rather than both measures appearing together on a single NULL-key row. No data is lost, so Semantic 3 ("every group appears") is technically satisfied either way, but the grouping itself is wrong whenever the shared key is nullable: what should be one row for "unknown region" becomes two. NULL-safe equality on the join predicate is what makes the merge produce exactly one row per shared-key value, NULL included. This only matters when the stitching dimension's key is actually nullable; engines MAY use plain equality when the model declares the key non-nullable.
